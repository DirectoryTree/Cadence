...
it('computes the next daily occurrence', function () {
    ...
    $next = (new RruleSchedule('FREQ=DAILY;DTSTART=20260501T090000'))->getNextOccurrence(now());
...
it('computes the next weekly occurrence', function () {
    ...
    $next = (new RruleSchedule('FREQ=WEEKLY;BYDAY=MO;DTSTART=20260427T090000'))->getNextOccurrence(now());
...
it('computes the next monthly occurrence', function () {
    ...
    $next = (new RruleSchedule('FREQ=MONTHLY;BYMONTHDAY=1;DTSTART=20260101T090000'))->getNextOccurrence(now());
...
it('returns null when rule is exhausted', function () {
    ...
    $next = (new RruleSchedule('FREQ=DAILY;COUNT=3;DTSTART=20260501T090000'))->getNextOccurrence(now());
...
it('computes next occurrence with timezone', function () {
    ...
    $next = (new RruleSchedule('FREQ=DAILY;DTSTART=20260501T090000', 'America/New_York'))->getNextOccurrence(now());
...
it('computes next occurrence with timezone set via method', function () {
    ...
    $next = (new RruleSchedule('FREQ=DAILY;DTSTART=20260501T090000'))
...
it('serializes to expression', function () {
    expect((new RruleSchedule('FREQ=DAILY;COUNT=5'))->toExpression())->toBe('FREQ=DAILY;COUNT=5');
});

it('reconstitutes from expression', function () {
    Carbon::setTestNow('2026-05-02 10:00:00');

    $next = RruleSchedule::fromExpression('FREQ=DAILY;DTSTART=20260501T090000')->getNextOccurrence(now());

    expect($next->format('Y-m-d H:i:s'))->toBe('2026-05-03 09:00:00');
});

it('returns the timezone', function () {
    expect((new RruleSchedule('FREQ=DAILY'))->getTimezone())->toBeNull();
    expect((new RruleSchedule('FREQ=DAILY', 'America/New_York'))->getTimezone())->toBe('America/New_York');
});
